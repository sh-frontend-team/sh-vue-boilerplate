...
export function formatFriendlyDate(date, format) {
    ...
    switch (now.year() - d.year()) {
        case 0:
            break;
        case 1:
            return d.format(`[去年]M[月]D[日] ${TIME_FORMAT}`);
        case 2:
            return d.format(`[前年]M[月]D[日] ${TIME_FORMAT}`);
        default:
    ...
    switch (now.month() - d.month()) {
        case 0:
            break;
        default:
            return d.format(`M[月]d[日] ${TIME_FORMAT}`);
    }
    ...
    switch (now.date() - d.date()) {
        case 0:
            break;
        case 1:
            return d.format(`[昨天] ${TIME_FORMAT}`);
        case 2:
            return d.format(`[前天] ${TIME_FORMAT}`);
        default:
            return d.format(`M[月]d[日] ${TIME_FORMAT}`);
    }
